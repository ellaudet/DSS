...
mean(bes1$leave) - 1.96 * sqrt(var(bes1$leave) / n) # lower limit
mean(bes1$leave) + 1.96 * sqrt(var(bes1$leave) / n) # upper limit
...
mean(treatment$reading) - mean(control$reading) - 
  1.96 * sqrt(var(treatment$reading) / n_t 
              + var(control$reading) / n_c) # lower limit

mean(treatment$reading) - mean(control$reading) + 
  1.96 * sqrt(var(treatment$reading) / n_t 
              + var(control$reading) / n_c) # upper limit
...
co$gdp_change <- 
  ((co$gdp - co$prior_gdp) / co$prior_gdp) * 100 # creates GDP percentage change variable
co$light_change <- 
  ((co$light - co$prior_light) / co$prior_light) * 100 # creates light percentage change variable

## Fit linear model
fit <- lm(gdp_change ~ light_change, data=co) # fits and store linear model
...
## Fit linear model
fit <- lm (pro_russian_vote ~ russian_tv + within_25km, data=uas) # fits and stores linear model
fit # shows contents of object
